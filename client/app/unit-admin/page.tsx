...
function UnitAdminLoginForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); setError(''); setIsLogging(true); const result = await login(formData.email, formData.password); if (result.success) router.push('/unit-admin/dashboard'); else { setError(result.error || 'Login failed'); setIsLogging(false); }};
