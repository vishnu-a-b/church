...
export default function ThirukkarmangalBookingsPage() {
  ...
  const clearFilters = () => {
    ...
    setLoading(true);
    api.get('/thirukkarmangal/scheduled-bookings')
  ...
  const handlePay = async () => {
    ...
    try {
      await api.put(`/thirukkarmangal/scheduled-bookings/${payModalBooking._id}/payment`, {
        ...
        receivingLedgerId: payLedger || undefined,
      });
                ...
                <div key={b._id} className="flex items-center justify-between bg-white rounded-lg px-4 py-2 border border-amber-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs bg-teal-600 text-white px-3 py-1 rounded-lg hover:bg-teal-700"
                    >
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            tab === 'list' ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-600 hover:text-gray-800'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            tab === 'calendar' ? 'bg-white text-teal-700 shadow-sm' : 'text-gray-600 hover:text-gray-800'
          }`}
        >
      ...
      {tab === 'list' && (
        <>
          ...
          <form onSubmit={handleSearch} className="bg-white rounded-lg shadow p-4 flex flex-wrap gap-3 items-end">
            ...
            <button type="submit" className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 text-sm">Search</button>
          </form>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {loading ? (
              ...
              <div className="flex flex-col items-center py-16 gap-2 text-gray-400">
                ...
                <Link href="/church-admin/dashboard/thirukkarmangal/bookings/new" className="text-sm text-teal-600 hover:underline">
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap">
                          {b.status === 'pending' && (
                            <>
                              <button
                                onClick={() => openPayModal(b)}
                                className="inline-flex items-center gap-1 text-xs bg-teal-600 text-white px-3 py-1.5 rounded-lg hover:bg-teal-700 mr-2"
                              >
              ...
              {calDays.map((day, i) => {
                ...
                return (
                  <div
                    ...
                    className={`h-14 border-t border-r border-gray-100 p-1 cursor-pointer hover:bg-teal-50 transition-colors ${
                    ...
                    {selectedDayBookings.map(b => (
                      <div key={b._id} className="border border-gray-200 rounded-lg p-3">
                        ...
                        {b.status === 'pending' && (
                          <button
                            onClick={() => openPayModal(b)}
                            className="mt-2 w-full flex items-center justify-center gap-1 text-xs bg-teal-600 text-white px-3 py-1.5 rounded-lg hover:bg-teal-700"
                          >
                            <Receipt className="w-3 h-3" /> Add Payment
                          </button>
                        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              {edvLedgers.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Receiving Account <span className="text-gray-400 font-normal">(optional)</span></label>
                  <select value={payLedger} onChange={(e) => setPayLedger(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2">
                    <option value="">— None (use default) —</option>
                    {edvLedgers.map(l => <option key={l.id} value={l.id}>{l.group.name} › {l.name}</option>)}
                  </select>
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  onClick={handlePay}
                  disabled={paying}
                  className="flex items-center gap-2 px-5 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:opacity-50"
                >
