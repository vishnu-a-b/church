...
export default function ChurchAdminEdvSyncPage() {
  ...
  const fetchPending = async () => {
    setLoading(true);
    try {
      const response = await api.get('/edv-sync/pending');
  ...
  const handleRetry = async (id: string) => {
    ...
    try {
      ...
      fetchPending(); // refresh to pick up the updated error message
    } finally {
      setRetryingId(null);
    }
  };
  ...
  const handleRetryAll = async () => {
    setRetryingAll(true);
    try {
      const response = await api.post('/edv-sync/retry-all');
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          disabled={retryingAll || pending.length === 0}
