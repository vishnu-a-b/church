...
export default function CampaignsPage() {
  ...
  const handleOpenPaymentModal = (campaign: Campaign) => {
    ...
    setPaymentData({ memberId: '', amount: '', receivingLedgerId: '' });
    setPaymentErrors({});
    setSelectedUnit('');
    setSelectedBavanakutayima('');
    setSelectedHouse('');
  ...
  const handleAddPayment = async () => {
    ...
    try {
      await api.post(`/campaigns/${selectedCampaign._id}/contribute`, {
        ...
        receivingLedgerId: paymentData.receivingLedgerId || undefined,
      });
            ...
            <div className="space-y-4">
              ...
              {edvLedgers.length > 0 && (
                <div>
                  ...
                  <select
                    value={paymentData.receivingLedgerId}
                    onChange={(e) => setPaymentData({ ...paymentData, receivingLedgerId: e.target.value })}
                    className="w-full border border-gray-300 rounded-lg px-4 py-2"
                  >
                    <option value="">— None (use default) —</option>
                    {edvLedgers.map(l => (
