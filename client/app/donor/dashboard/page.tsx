...
export default function DonorDashboardPage() {
  ...
  const fetchDues = async () => {
    ...
    try {
      ...
      toast.error('Failed to load your contribution history');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800">My Monthly Contribution</h2>
        <p className="text-gray-600">Your pledge history as an outside supporter</p>
      </div>
      ...
      <p className="text-xs text-gray-400">
        Payments are recorded by the church office when received — this page is for tracking only.
      </p>
    </div>
  );
}
