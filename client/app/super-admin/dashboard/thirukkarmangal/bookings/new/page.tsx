...
function NewBookingContent() {
  ...
  useEffect(() => {
    setRites([]); setUnits([]);
    setSelectedRiteId(''); setSelectedUnit(''); setSelectedBk(''); setSelectedHouse(''); setSelectedMember('');
    setBavanakutayimas([]); setHouses([]); setMembers([]);
    if (!selectedChurchId) return;
    Promise.all([
      api.get(`/thirukkarmangal/rites?churchId=${selectedChurchId}`),
      api.get(`/units?churchId=${selectedChurchId}`),
    ]).then(([rRes, uRes]) => {
      setRites(rRes.data?.data || []);
      setUnits(uRes.data?.data || []);
    }).catch(() => toast.error('Failed to load church data'));
  }, [selectedChurchId]);
  ...
  const backHref = selectedChurchId
    ? `/super-admin/dashboard/thirukkarmangal/bookings?churchId=${selectedChurchId}`
    : '/super-admin/dashboard/thirukkarmangal/bookings';
...
export default function SuperAdminNewThirukkarmangalBookingPage() {
  return (
