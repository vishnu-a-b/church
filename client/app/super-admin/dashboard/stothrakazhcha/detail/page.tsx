...
function DetailContent() {
  ...
  const approveGroup = async (group: BkGroup) => {
    ...
    if (!confirm(`Settle all ${group.pendingCount} pending contribution(s) for ${group.bavanakutayimaName}?\n\nTotal: ₹${group.totalAmount.toLocaleString('en-IN')}`)) return;
    ...
    try {
      ...
      toast.success(`${group.pendingCount} contribution(s) settled`);
      fetchDetail();
    } catch (e: any) {
      toast.error(e.response?.data?.error || 'Failed to settle group');
    } finally {
      setActingId(null);
    }
  };

  if (!id) return <div className="text-center py-16 text-gray-500">No week selected.</div>;
  ...
  if (!week) return <div className="text-center py-16 text-gray-500">Week not found.</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-bold text-gray-800">
          Week {week.weekNumber}, {week.year} — Settle by Group
        </h2>
        ...
        <div className="flex items-center justify-between gap-4">
            ...
            {extraForm ? (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <input
                  ...
                  className="w-28 border border-purple-400 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                  placeholder="0"
                  autoFocus
                />
                <input
                  ...
                  className="w-56 border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
          ...
          {!extraForm && (
            <button
              ...
              className="flex items-center gap-1.5 text-purple-600 hover:text-purple-700 text-sm font-medium border border-purple-200 rounded-lg px-3 py-2 hover:bg-purple-50 shrink-0"
            >
          ...
          return (
            <div key={groupKey} className="bg-white rounded-xl shadow overflow-hidden">
              ...
              <button
                ...
                className="w-full flex items-center justify-between px-6 py-4 bg-purple-50 border-b border-purple-100 hover:bg-purple-100 transition-colors text-left"
              >
                <div className="flex items-center gap-3 min-w-0">
                  {isExpanded
                    ? <ChevronDown className="w-5 h-5 text-purple-500 shrink-0" />
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      ...
                      {' '}&middot;{' '}
                      <span className="text-green-600 font-medium">{group.approvedCount} settled</span>
                ...
                <div className="flex items-center gap-3 shrink-0" onClick={(e) => e.stopPropagation()}>
                  {group.pendingCount > 0 && (
                    <button
                      ...
                      className="flex items-center gap-2 bg-purple-600 text-white px-5 py-2.5 rounded-lg hover:bg-purple-700 disabled:opacity-50 font-semibold text-sm"
                    >
                      <CheckCircle className="w-4 h-4" />
                      {isActing ? 'Settling...' : 'Settle All'}
                    </button>
                  )}
                    ...
                    <div>
                      ...
                      <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
                        ...
                        {entry.approvalStatus === 'pending_approval' ? 'Pending' :
                         entry.approvalStatus === 'approved' ? 'Settled' : 'Rejected'}
                      </span>
                    </div>
                      ...
                      {isEditing ? (
                        <>
                          ...
                          <input
                            ...
                            className="w-24 border border-purple-400 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                            autoFocus
                          />
                          <button
...
export default function SuperAdminStothrakazhchaDetailPage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center py-16">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600" />
