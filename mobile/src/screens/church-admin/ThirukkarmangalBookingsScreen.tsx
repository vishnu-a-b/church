...
export default function ChurchAdminThirukkarmangalBookingsScreen() {
  ...
  const fetchBookings = useCallback(async (riteId: string, from: Date | null, to: Date | null) => {
    try {
  ...
  useEffect(() => {
    api.get('/thirukkarmangal/rites').then((r) => setRites(r.data?.data || [])).catch(console.error);
    fetchBookings('', null, null);
  }, [fetchBookings]);

  const handleSearch = () => fetchBookings(filterRiteId, fromDate, toDate);
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            {hasFilters ? (
              ...
              <Text style={styles.emptySub}>No Thirukkarmangal payments have been recorded yet</Text>
            )}
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  splitItem: { fontSize: 11, color: COLOR, backgroundColor: '#f0fdf4', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
