...
const seedDatabase = async () => {
  try {
    ...
    const unit1Admin = await User.create({
      username: 'unit1admin',
    ...
    const unit2Admin = await User.create({
      username: 'unit2admin',
    ...
    const kutayimaAdmin1 = await User.create({
    ...
    const kutayimaAdmin2 = await User.create({
    ...
    await Campaign.findByIdAndUpdate(campaigns[0]._id, {
      totalCollected: 1000,
      participantCount: 2,
    });

    await Campaign.findByIdAndUpdate(campaigns[1]._id, {
      totalCollected: 1000,
      participantCount: 1,
    });
    ...
    for (const house of houses) {
      const houseTransactions = transactions.filter(
        (t) => t.houseId?.toString() === house._id.toString()
      );
    ...
    await SpiritualActivity.insertMany(spiritualActivities);
