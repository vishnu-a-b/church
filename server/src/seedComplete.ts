...
const seedDatabase = async () => {
  try {
    ...
    const dues = await StothrakazhchaDue.insertMany([
      {
        churchId: church._id,
    ...
    const stothraTransaction = await Transaction.create({
      churchId: church._id,
      amount: 100,
    ...
    console.log(`   Login at: /member-login (use username)`);console.log(`\n💡 Note: Different login pages accept different formats:`);
