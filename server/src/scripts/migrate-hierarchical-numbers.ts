...
async function migrateUnits() {
  console.log('\n=== Migrating Units ===');
  const units = await Unit.find().sort({ churchId: 1, unitNumber: 1 });
  ...
  for (const unit of units) {
    const church = await Church.findById(unit.churchId);
    if (!church) {
...
async function migrateBavanakutayimas() {
  ...
  const bavanakutayimas = await Bavanakutayima.find().sort({ unitId: 1, bavanakutayimaNumber: 1 });
  ...
  for (const bk of bavanakutayimas) {
    const unit = await Unit.findById(bk.unitId);
    if (!unit) {
    ...
    await Bavanakutayima.updateOne(
      { _id: bk._id },
      { $set: { uniqueId: newUniqueId } }
    );
...
async function migrateHouses() {
  console.log('\n=== Migrating Houses ===');
  const houses = await House.find().sort({ bavanakutayimaId: 1, houseNumber: 1 });
  ...
  for (const house of houses) {
    const bk = await Bavanakutayima.findById(house.bavanakutayimaId);
    if (!bk) {
...
async function migrateMembers() {
  console.log('\n=== Migrating Members ===');
  const members = await Member.find().sort({ houseId: 1, memberNumber: 1 });
  ...
  for (const member of members) {
    const house = await House.findById(member.houseId);
    if (!house) {
...
async function validateMigration() {
  ...
  const collections = [
    { name: 'Churches', model: Church },
    { name: 'Units', model: Unit },
    { name: 'Bavanakutayimas', model: Bavanakutayima },
  ...
  const invalidChurches = await Church.find({ uniqueId: { $not: /^\d+$/ } });
  ...
  const invalidUnits = await Unit.find({ uniqueId: { $not: /^\d+-\d+$/ } });
  ...
  const invalidBKs = await Bavanakutayima.find({ uniqueId: { $not: /^\d+-\d+-\d+$/ } });
  ...
  const invalidHouses = await House.find({ uniqueId: { $not: /^\d+-\d+-\d+-\d+$/ } });
  ...
  const invalidMembers = await Member.find({ uniqueId: { $not: /^\d+-\d+-\d+-\d+-\d+$/ } });
